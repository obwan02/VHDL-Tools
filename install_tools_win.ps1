...
if( !(Get-Command ghdl -ErrorAction SilentlyContinue) ) {
	...
	Write-Host "`$env:PATH = `$env:PATH + ';' + '$HOME\Programs\GHDL\bin'" | Add-Content $PROFILE.CurrentUserAllHosts -Encoding UTF8
} else {
...
if( !(Get-Command gtkwave -ErrorAction SilentlyContinue) ) {
	...
	Write-Host "`$env:PATH = `$env:PATH + ';' + '$HOME\Programs\just'" | Add-Content $PROFILE.CurrentUserAllHosts -Encoding UTF8
} else {
...
if( !(Get-Command gtkwave -ErrorAction SilentlyContinue) ) {
	...
	Write-Host "`$env:PATH = `$env:PATH + ';' + '$HOME\Programs\GTKWave'" | Add-Content $PROFILE.CurrentUserAllHosts -Encoding UTF8
} else {
